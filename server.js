...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/scripts' && method === 'GET') {
    try {
      ...
      const scanDir = (dir, relPath = '') => {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (!scriptMap.has(itemRel)) {
              scriptMap.set(itemRel, {
                path: itemRel,
                name: entry.name,
      ...
      const releaseScripts = path.join(RELEASE_DIR, 'scripts');
      if (fs.existsSync(releaseScripts)) scanDir(releaseScripts);

      if (fs.existsSync(path.join(ROOT_DIR, 'Ringmastered.lua')) && !scriptMap.has('Ringmastered.lua')) {
        scriptMap.set('Ringmastered.lua', {
          path: 'Ringmastered.lua',
          name: 'Ringmastered.lua',
          size: fs.statSync(path.join(ROOT_DIR, 'Ringmastered.lua')).size
        });
      }

      const scripts = Array.from(scriptMap.values()).sort((a, b) => a.path.localeCompare(b.path));
  ...
  if (pathname === '/api/scripts/read' && method === 'GET') {
    ...
    if (!fs.existsSync(targetPath)) {
      targetPath = path.join(RELEASE_DIR, 'scripts', scriptRel);
    }
  ...
  if (pathname === '/api/scripts/delete' && (method === 'POST' || method === 'DELETE')) {
    const body = await getBody();
    const scriptRel = body.path || parsedUrl.query.path;
    if (!scriptRel) return jsonResponse({ error: 'Missing path' }, 400);
    ...
    try {
      ...
      return jsonResponse({ success: true, message: `Script ${scriptRel} deleted.` });
    } catch (e) {
      return jsonResponse({ error: e.message }, 500);
    }
  }
